...
import {
  convertPackageBundleCurrencyAmount as convertCurrencyAmount,
...
export interface PackageBundlePaymentFormProps {
  type: string;
  entityId: string;
  entityNumber: string;
  title: string;
...
function PackageBundlePaymentFormContent({
  ...
  const [customAmount, setCustomAmount] = useState<string>(
    hasQueryAmount ? Number(queryAmount).toFixed(2) : ""
  );
  ...
  useEffect(() => {
    ...
    const fetchCountries = async () => {
      try {
        ...
        console.error("Failed to load countries in PackageBundlePaymentForm:", err);
      }
    };
  ...
  // Active currency converted figures
  const convertedSubtotal = convertCurrencyAmount(projectSubtotal, currency, nativeCurrency || "USD", conversionRate);
  const convertedAmountPaid = convertCurrencyAmount(amountPaid, currency, nativeCurrency || "USD", conversionRate);
  const activeVatRate = getActiveVatRate();
  const activeVatAmount = activeVatRate > 0 ? (convertedSubtotal * activeVatRate) / 100 : 0;
  ...
  useEffect(() => {
    ...
    if (paramAmount && !isNaN(Number(paramAmount)) && Number(paramAmount) > 0) {
      ...
      setPaymentOption("custom");
      setCustomAmount(activePendingBalance.toFixed(2));
    }
  }, [searchParams, amountPaid, activePendingBalance]);
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    if (paymentOption === "custom") {
      ...
      } else if (parseFloat(customAmount) > activePendingBalance + 0.01) {
        errors.amount = `Amount cannot exceed pending balance (${formatActiveCurrency(activePendingBalance, currency)}).`;
      }
    }
  ...
  return (
    <div className={containerClassName !== undefined ? containerClassName : "bg-white border border-gray-300 rounded-lg p-4 sm:p-6 md:p-8"}>
            ...
            <div className="w-full sm:w-60 shrink-0 order-1 lg:order-2 space-y-1.5 text-xs sm:text-sm">
              <div className="flex justify-between items-center font-semibold">
                ...
                <span className="text-gray-900 font-bold">
                  {formatActiveCurrency(activeTotalWithVat, currency)}
                </span>
              </div>

              <div className={`flex justify-between items-center pt-1 border-t border-gray-100 font-semibold ${activePendingBalance > 0.009 ? "text-red-600" : "text-gray-600"}`}>
                <span>Pending Balance</span>
                <span className="font-bold">
                  {formatActiveCurrency(activePendingBalance, currency)}
                </span>
              </div>
            </div>
          </div>
          ...
          <table className="w-full min-w-[500px] sm:min-w-0 table-fixed">
            <thead>
              <tr className="border-b border-gray-200 bg-white">
                <th className="text-left py-3 px-3 sm:px-6 text-xs font-bold text-gray-500 uppercase tracking-wider w-[55%] sm:w-[58%]">Item</th>
                <th className="text-left py-3 px-3 sm:px-6 text-xs font-bold text-gray-500 uppercase tracking-wider w-[25%] sm:w-[22%] whitespace-nowrap">Duration</th>
                <th className="text-right py-3 px-3 sm:px-6 text-xs font-bold text-gray-500 uppercase tracking-wider w-[20%] whitespace-nowrap">Amount</th>
              </tr>
            </thead>
            <tbody>
              {regularItems.map((item, idx) => (
                <tr key={`regular-${idx}-${item.amount ?? 0}`} className="border-b border-gray-100 last:sm:border-b-0">
                  ...
                  <td className="py-4 px-3 sm:px-6 text-xs sm:text-sm text-gray-500 whitespace-nowrap text-left">
              ...
              {activeVatRate > 0 && activeVatAmount > 0 && (
                <tr className="border-t-2 border-gray-200 bg-gray-50/70">
                  ...
                  <td className="py-2.5 px-3 sm:px-6 text-right text-xs font-semibold text-gray-700">
                    {formatActiveCurrency(convertedSubtotal, currency)}
                  </td>
                </tr>
              )}
              {activeVatRate > 0 && activeVatAmount > 0 && (
      ...
      {(!isFullyPaid || (amountPaid > 0 && activePendingBalance > 0.009)) && (
        <div>
                      ...
                      <p className="text-xs text-gray-500">
                        {availableCredits > 0 ? (
                          <span className="flex items-center gap-1">
                            Apply your balance of <span className="font-bold text-gray-800">{formatActiveCurrency(availableCredits, currency)}</span> to
                            this payment.
                          </span>
                        ) : (
