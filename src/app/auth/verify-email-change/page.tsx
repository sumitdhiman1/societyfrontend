...
function VerifyEmailChangeContent() {
  ...
  useEffect(() => {
    ...
    const verify = async () => {
      ...
      try {
        const res = await profileService.verifyEmailChange(token);
        ...
        setMessage(
          err?.response?.data?.message ||
            err?.message ||
            "An unexpected error occurred while verifying your new email address."
        );
      }
    };
        ...
        {status === "error" && (
          <div className="flex flex-col items-center py-4">
            ...
            <div className="flex flex-col gap-3 w-full">
              ...
              <Link
                href="/help-support"
                className="text-[#4545F0] hover:underline font-semibold text-xs transition-colors"
              >
                Contact Support
              </Link>
            </div>
    ...
    <Suspense
      fallback={
        <div className="min-h-screen flex items-center justify-center bg-[#F8FAFC]">
          <div className="w-16 h-16 border-4 border-[#4545F0]/20 border-t-[#4545F0] rounded-full animate-spin"></div>
        </div>
      }
    >
