...
const LOGO_SVG = `
  <svg width="328" height="104" viewBox="0 0 328 104" fill="none" xmlns="http://www.w3.org/2000/svg" style="display: block; width: 328px; height: 104px;">
        ...
        .map((opt) => {
          ...
          return `
            <article class="scope-item">
              <h3 class="question-title">${escapeHtml(cleanQuestion)}</h3>
              ${hasMultiple
                ? `<ul class="answer-list">
                    ${opt.answers
                      .map((ans) => `<li><span style="position: absolute; left: 0; color: #202124;">•</span>${escapeHtml(ans.replace(/^[•\-\*]\s*/, ""))}</li>`)
                      .join("")}
                  </ul>`
                : `<p class="answer-text">${escapeHtml(opt.answers[0] || "—")}</p>`
              }
            </article>
          `;
        })
  ...
  return `
    <div style="margin-top: auto;">
      ...
      <aside class="summary-card">
        <div class="summary-row summary-timeline">
          <span class="summary-label">ESTIMATED TIMELINE</span>
          <span class="summary-value">${escapeHtml(d.duration)}</span>
        </div>
        <div class="summary-row summary-investment">
          <span class="summary-label">INVESTMENT TOTAL</span>
          <span class="summary-value">${escapeHtml(d.formattedPrice)}</span>
        </div>
      </aside>
      ...
      <footer class="proposal-footer">
        <div class="footer-disclaimers">
          <p class="disclaimer-text">Acceptance of this quote binds the client to the agreed delivery timeline and total investment.</p>
          <p class="disclaimer-text disclaimer-highlight">Note: Time spent waiting for client replies does not count towards project deadlines.</p>
          <p class="disclaimer-text">For inquiries, please reach out to <a href="mailto:contact@societywebsolutions.com">contact@societywebsolutions.com</a></p>
        </div>
        <div style="display: flex; justify-content: ${totalPages > 1 ? "space-between" : "center"}; align-items: center; width: 100%;">
          <div class="footer-watermark">SOCIETY WEB SOLUTIONS</div>
          ${totalPages > 1 ? `<div style="font-family: 'Inter', sans-serif; font-size: 20px; font-weight: 600; color: #879095;">Page ${currentPage} of ${totalPages}</div>` : ""}
        </div>
      </footer>
    </div>
  `;
    ...
    <div style="margin-top: auto;">
      ...
      <footer class="proposal-footer">
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; width: 100%;">
          <div class="footer-watermark">SOCIETY WEB SOLUTIONS</div>
          <div style="font-family: 'Inter', sans-serif; font-size: 20px; font-weight: 600; color: #879095;">Page ${currentPage} of ${totalPages}</div>
        </div>
      </footer>
    </div>
  `;
...
function renderPreparedForClientBlock(d: CalculatorPDFData): string {
  const emailLine =
    d.clientEmail && !d.hideClientEmail
      ? `<div class="client-email">${escapeHtml(d.clientEmail)}</div>`
      : "";
  return `
    <div class="quote-client">
      <h2 class="card-meta-label">PREPARED FOR</h2>
...
function renderScopeOverviewIntro(d: CalculatorPDFData): string {
  const text =
    ...
    d.description ||
    d.subtitle ||
    "By combining intuitive design with robust backend architecture, the project will enhance efficiency and reduce friction in daily workflows.";
...
function estimateOptionHeight(opt: { question: string; answers: string[] }): number {
  ...
  const ansLines = answers.reduce((acc, a) => acc + Math.max(1, Math.ceil(a.length / 80)), 0);
  const titleHeight = qLines * 38;
  const contentHeight = hasMultiple ? ansLines * 43 : ansLines * 39;
  return titleHeight + 18 + contentHeight + 56;
}
...
function paginateCalculatorOptions(
  ...
  // Single page document check (Page 1 top = ~871px, Summary card + footer = ~496px, remaining = ~307px)
  if (totalHeight <= 310) {
    return [options];
  }
  ...
  // Page 1 capacity: 580px
  const page1: Array<{ question: string; answers: string[] }> = [];
  let p1Height = 0;
  while (remaining.length > 0) {
    const nextH = estimateOptionHeight(remaining[0]);
    if (page1.length > 0 && p1Height + nextH > 580) {
      break;
    }
  ...
  // Subsequent pages
  while (remaining.length > 0) {
    const remainingH = remaining.reduce((sum, opt) => sum + estimateOptionHeight(opt), 0);
    // If all remaining options fit on the last page capacity (940px)
    if (remainingH <= 940) {
      pages.push(remaining);
      break;
    }

    // Otherwise, this middle page has 1220px capacity
    const currentPage: Array<{ question: string; answers: string[] }> = [];
    let curH = 0;
    while (remaining.length > 0) {
      const nextH = estimateOptionHeight(remaining[0]);
      if (currentPage.length > 0 && curH + nextH > 1220) {
        break;
      }
      currentPage.push(remaining.shift()!);
      curH += nextH;
    }
    pages.push(currentPage);
  }

  // Safety guard for last page summary card space
  if (pages.length > 1) {
    const lastPage = pages[pages.length - 1];
    const lastPageH = lastPage.reduce((sum, opt) => sum + estimateOptionHeight(opt), 0);
    if (lastPageH > 940 && lastPage.length > 1) {
  ...
  const styleTag = `
    <style>
      ...
      *, *::before, *::after {
      ...
      .summary-timeline .summary-label {
        font-family: 'Inter', sans-serif;
        font-style: normal;
        font-weight: 700;
        font-size: 20px;
      ...
      .summary-timeline .summary-value {
        font-family: 'Inter', sans-serif;
        font-style: normal;
        font-weight: 700;
        font-size: 24px;
      ...
      .summary-investment .summary-label {
        font-family: 'Inter', sans-serif;
        font-style: normal;
        font-weight: 700;
        font-size: 22px;
      ...
      .summary-investment .summary-value {
        font-family: 'Inter', sans-serif;
        font-style: normal;
        font-weight: 700;
        font-size: 48px;
      ...
      <div>
        ...
        <header class="proposal-header">
          ...
          <div class="header-details">
            ...
            <div class="company-email">
              <a href="mailto:contact@societywebsolutions.com">contact@societywebsolutions.com</a>
            </div>
          </div>
        </header>
        ...
        <section class="quote-card">
          ...
          <div class="quote-meta">
            <h2 class="card-meta-label">QUOTE DETAILS</h2>
            <div class="meta-row">
              <span class="meta-label">Ref Number:</span>
              <span class="meta-value">${escapeHtml(d.projectNumber)}</span>
            </div>
            <div class="meta-row">
              <span class="meta-label">Issued On:</span>
              <span class="meta-value">${escapeHtml(d.issuedDate)}</span>
            </div>
            <div class="meta-row">
              <span class="meta-label">Valid Until:</span>
              <span class="meta-value meta-value-highlight">${escapeHtml(d.validUntilDate)}</span>
            </div>
          </div>
        </section>

        <!-- Project Scope Overview Section -->
        <section class="scope-overview">
          <h2 class="section-title">Project Scope Overview</h2>
          ${renderScopeOverviewIntro(d)}
        </section>
        `
            : `
        <!-- Secondary Header for Subsequent Pages -->
        <header class="proposal-header" style="padding-bottom: 24px; border-bottom: 2px solid #EEF0F5; margin-bottom: 36px; align-items: center;">
          <div class="header-logo">
            <div style="width: 240px; height: 76px; overflow: hidden;">
              ${LOGO_SVG.replace('width="328" height="104"', 'width="240" height="76"')}
            </div>
          </div>
          <div class="header-details" style="text-align: right;">
            <div style="font-family: 'Inter', sans-serif; font-weight: 700; font-size: 32px; line-height: 38px; color: #2A2AA0; letter-spacing: -0.02em;">PROJECT PROPOSAL</div>
            <div style="font-family: 'Inter', sans-serif; font-weight: 600; font-size: 20px; line-height: 26px; color: #879095; margin-top: 4px;">Ref: ${escapeHtml(d.projectNumber)} &bull; ${escapeHtml(d.categoryName)}</div>
          </div>
        </header>

        <section class="scope-overview" style="margin-bottom: 36px;">
          <h2 class="section-title" style="margin: 0;">Project Scope Overview (Continued)</h2>
        </section>
        `
        }

        <!-- Scope Questionnaire Items -->
...
export async function downloadCalculatorProjectPDF(data: any): Promise<void> {
  ...
  try {
    ...
    // Wait 250ms for layout and web fonts to settle
...
export function printCalculatorProjectPDF(data: any): void {
  ...
  const printContent = `
    ...
    <html>
      <head>
        <meta charset="utf-8" />
        <title>Project Proposal - ${escapeHtml(d.projectNumber)}</title>
        <style>
          @page {
            size: A4 portrait;
            margin: 0;
          ...
          * {
          ...
          .proposal-page {
            ...
            padding: 15mm 10mm 15mm !important;
